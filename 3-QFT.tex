...
\begin{equation}
  T_{haw}= \frac{1}{8\pi k_B M}.
\end{equation}


%Introducción relatividad general. Métrica Schwarzschild

Consireraremos el caso más sencillo de agujero negro, el agujero negro de Schwarzschild, que
...
corresponde con el vacío para un observador en caída libre muy alejado de la fuente
gravitacional.


La energía radiada por el agujero negro proviene de la materia que colapsa en el agujero negro.
...
de forma que la métrica fuera del objeto es la métrica de Minkowski y el vacío es el correspondiente
al espacio de Minkowski.

Una vez formadoo el agujero negro,todo el espacio viene descrito por la métrica de Schwarzschild y
el estado de vacío es el vacío de Boulware.

Considermos los modos de Boulware salientes que escapan la materia en colapso poco antes de formarse el horizonte.
...
\begin{equation}
  \phi^{u}_\omega = \frac{1}{\sqrt{4\pi\omega}}e^{i\omega 4M\ln \abs{\frac{u'}{4M}}}.
\end{equation}

Vemos que al aproximarse al horizonte, $u'\to 0$, la frequencia de estos modos tiende a infinito.
...
\begin{equation}
  \phi^{u}_\omega = \frac{1}{\sqrt{4\pi\omega}}e^{i\omega 4M\ln \abs{A(V_H-V)}}.
\end{equation}
